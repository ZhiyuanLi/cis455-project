...
/**
 * Language Detection Extension for Apache Nutch using Language Detection Library (http://code.google.com/p/language-detection/).
 * For HTMLLanguageParser and LanguageQueryFilter, the extensions of the Nutch's standard language-identifier plugin can be used
 * without modifications, so it is provides an extension of LanguageIdentifier only.
...
public class LanguageDetectionFilter implements IndexingFilter
{
	 ...
	 * @param inlinks - the links to the URL
	 */
	...
	{
		LuceneWriter.addFieldOptions("lang", LuceneWriter.STORE.YES, LuceneWriter.INDEX.UNTOKENIZED, conf);
	}
